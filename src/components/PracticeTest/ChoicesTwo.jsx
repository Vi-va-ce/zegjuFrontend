...
function ChoicesTwo({ data, currentIndex, onLocalStorageChange }) {
  ...
  const handleClick = (choiceId) => {
    if (selected === choiceId) {
      ...
      const sectionId = data.sections[0].section_id;
      const questionId = data.sections[0].question_groups[0].question_types[0].questions[currentIndex].question_id;
      const testId = data.test_id;
        ...
        {choices.map((choice, index) => (
          <li key={index}>
            <button onClick={() => handleClick(choice.choiceId)}>
              <div className={`bg-white md:w-[844px] w-[361px] mb-2 hover:bg-gray-200 ${selected === choice.choiceId ? 'bg-gray-400' : ''}`}>
